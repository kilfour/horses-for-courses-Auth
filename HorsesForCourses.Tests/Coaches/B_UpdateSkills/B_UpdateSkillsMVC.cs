...
public class B_UpdateSkillsMVC : CoachesMVCControllerTests
{
    ...
    public async Task UpdateSkills_POST_calls_the_service()
    {
        ...
        service.Verify(a => a.UpdateSkills(It.IsAny<Actor>(), TheCanonical.CoachId, TheCanonical.Skills));
    }
    ...
    {
        service.Setup(a => a.GetCoachDetail(It.IsAny<Actor>(), TheCanonical.CoachId)).ReturnsAsync(TheCanonical.CoachDetail());
        service.Setup(a => a.UpdateSkills(It.IsAny<Actor>(), It.IsAny<IdPrimitive>(), It.IsAny<List<string>>()))
