...
public class A_UpdateSkillsApi : CoachesApiControllerTests
{
    ...
    public async Task UpdateSkills_uses_the_service()
    {
        ...
        service.Verify(a => a.UpdateSkills(It.IsAny<Actor>(), TheCanonical.CoachId, It.Is<List<string>>(a => a == request.Skills)));
    }

    [Fact]
    public async Task UpdateSkills_Service_Success_Returns_NoContent()
    {
        service.Setup(a => a.UpdateSkills(It.IsAny<Actor>(), TheCanonical.CoachId, request.Skills)).ReturnsAsync(true);
        var response = await controller.UpdateSkills(TheCanonical.CoachId, request);
    ...
    {
        service.Setup(a => a.UpdateSkills(It.IsAny<Actor>(), TheCanonical.CoachId, request.Skills)).ReturnsAsync(false);
